...
public class characterJump : MonoBehaviour
{
    ...
    public GMTK_Settings_SO gmtkSettingsSO = null;
...
//    [SerializeField, Range(0f, 0.3f)][Tooltip("How far from ground should we cache your jump?")] public float jumpBuffer = 0.15f;
    ...
    void Awake()
    {
        if (gmtkSettingsSO == null) {
            Debug.LogError( "You must assign a GMTK_Settings_SO Scriptable Object to the CharacterJump script for it to function." );
            enabled = false;
        }
    ...
    private void setPhysics() {
        ...
        Vector2 newGravity = new Vector2( 0, ( -2 * gmtkSettingsSO.jumpHeight ) / ( gmtkSettingsSO.timeToJumpApex * gmtkSettingsSO.timeToJumpApex ) );
    ...
    private void FixedUpdate()
    {
        ...
        if ( gmtkSettingsSO.jumpBuffer > 0 ) {
            //Instead of immediately turning off "desireJump", start counting up...
            //All the while, the DoAJump function will repeatedly be fired off
            if ( desiredJump ) {
                jumpBufferCounter += Time.deltaTime;

                if ( jumpBufferCounter > gmtkSettingsSO.jumpBuffer ) {
    ...
    private void calculateGravity()
    {
        ...
        if ( velocity.y > 0.01f ) //if (rigid.velocity.y > 0.01f)
        {
            ...
            else
            {
                ...
                if ( gmtkSettingsSO.variablejumpHeight )
                {
                    //Apply upward multiplier if player is rising and holding jump
                    if (pressingJump && currentlyJumping)
                    {
                        gravMultiplier = gmtkSettingsSO.upwardMovementMultiplier;
                    }
                    //But apply a special downward multiplier if the player lets go of jump
                    else
                    {
                        gravMultiplier = gmtkSettingsSO.jumpCutOff;
                    }
                }
                else
                {
        ...
        else if ( velocity.y < -0.01f ) //else if (rigid.velocity.y < -0.01f)
        {
            ...
            else
            {
                ...
                gravMultiplier = gmtkSettingsSO.downwardMovementMultiplier;
            }
        ...
        velocity.y = Mathf.Clamp( velocity.y, -gmtkSettingsSO.speedLimit, 100 );
    ...
    private void DoAJump()
    {
        ...
        if (onGround || (coyoteTimeCounter > 0.03f && coyoteTimeCounter < gmtkSettingsSO.coyoteTime ) || canJumpAgain)
        {
            desiredJump = false;
            jumpBufferCounter = 0;
            coyoteTimeCounter = 0;
            ...
            canJumpAgain = ( gmtkSettingsSO.maxAirJumps == 1 && canJumpAgain == false);

            //Determine the power of the jump, based on our gravity and stats
            jumpSpeed = Mathf.Sqrt(-2f * Physics2D.gravity.y * rigid.gravityScale * gmtkSettingsSO.jumpHeight );
            if (jumpSpeed > 100) {
